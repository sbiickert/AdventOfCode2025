...
struct AoCInput {
	...
	public static func readInputFile(named fileName: String, removingEmptyLines removeEmpty:Bool) -> [String] {
		...
		if removeEmpty {
			...
			while let last = results.last, last.isEmpty {
	...
	public var allInputGroups: [[String]] {
		return AoCInput.readGroupedInputFile(named: fileName);
	}
}
