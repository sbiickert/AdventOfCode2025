...
// Immutable, computed once on first use. Globals initialised this way are
// thread-safe; a lazily-filled dictionary would race under parallel access.
private let _coord3DRookOffsets: [AoCCoord3D] = [AoCDir3D.up.offset, AoCDir3D.down.offset,
												 AoCDir3D.left.offset, AoCDir3D.right.offset,
												 AoCDir3D.forward.offset, AoCDir3D.back.offset]

/// The eight corner cells: one step on *every* axis. Cells that share only an
/// edge (two axes) are deliberately not included, matching `isAdjacent`.
private let _coord3DBishopOffsets: [AoCCoord3D] = {
	var offsets = [AoCCoord3D]()
	for x in [-1, 1] {
		for y in [-1, 1] {
			for z in [-1, 1] {
				offsets.append(AoCCoord3D(x: x, y: y, z: z))
			}
		}
	}
	return offsets
...
private let _coord3DQueenOffsets: [AoCCoord3D] = _coord3DRookOffsets + _coord3DBishopOffsets
...
struct AoCCoord3D: Hashable, Equatable, CustomDebugStringConvertible {
	...
	static func getAdjacentOffsets(rule: AoCAdjacencyRule = .rook) -> [AoCCoord3D] {
		switch rule {
		case .rook:   return _coord3DRookOffsets
		case .bishop: return _coord3DBishopOffsets
		case .queen:  return _coord3DQueenOffsets
		}
	}
...
// Immutable, computed once on first use. Globals initialised this way are
// thread-safe; a lazily-filled dictionary would race under parallel access.
private let _coordRookOffsets: [AoCCoord2D] = [AoCDir.north.offset, AoCDir.east.offset,
											   AoCDir.south.offset, AoCDir.west.offset]

private let _coordBishopOffsets: [AoCCoord2D] = [AoCDir.nw.offset, AoCDir.ne.offset,
												 AoCDir.sw.offset, AoCDir.se.offset]

private let _coordQueenOffsets: [AoCCoord2D] = _coordRookOffsets + _coordBishopOffsets
...
struct AoCCoord2D: Hashable, Equatable, CustomDebugStringConvertible {
	...
	static func getAdjacentOffsets(rule: AoCAdjacencyRule = .rook) -> [AoCCoord2D] {
		switch rule {
		case .rook:   return _coordRookOffsets
		case .bishop: return _coordBishopOffsets
		case .queen:  return _coordQueenOffsets
		}
	}
